...
public class LibraryTableModel extends AbstractTableModel {
    ...
    public static trie songsTrie = new trie();

    public void changeSongs(Vector<Song> songs) {
	songListing.removeAllElements();
	for(int idx=0; idx<songs.size(); idx++ )
	    songListing.add(songs.get(idx));
	resetIdx();
	...
	for (int idx = 0; idx < songs.size(); idx++)
	    songsTrie.addSong(new songPtr(songs.get(idx), idx));
